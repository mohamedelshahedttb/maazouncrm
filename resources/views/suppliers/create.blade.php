...
<div class="p-6">
    <!-- Page Title -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800">إضافة مورد جديد</h1>
        <p class="text-gray-600 mt-2">املأ التفاصيل لتسجيل مورد جديد في النظام</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <form method="POST" action="{{ route('suppliers.store') }}" enctype="multipart/form-data" class="p-6">
            @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="rating" id="rating" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <option value="4" {{ old('rating') == '4' ? 'selected' : '' }}>4 - ممتاز</option>
                            <option value="5" {{ old('rating') == '5' ? 'selected' : '' }}>5 - رائع</option>
                        </select>
                        @error('rating')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            <div class="flex justify-end space-x-4 space-x-reverse mt-8 pt-6 border-t border-gray-200">
                ...
                <button type="submit" class="px-6 py-2 bg-blue-500 text-blue-100 rounded-lg hover:bg-blue-600 hover:text-white font-medium">
                    حفظ المورد +
                </button>
            </div>
        </form>
