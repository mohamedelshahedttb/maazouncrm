...
<div class="p-6">
    <!-- Page Title -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-blue-700">تعديل المستخدم</h1>
        <p class="text-gray-600 mt-2">تعديل بيانات المستخدم: {{ $user->name }}</p>
    </div>

    <div class="max-w-4xl mx-auto">
        <form action="{{ route('users.update', $user) }}" method="POST" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="role" id="role" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        ...
                        <option value="partner" {{ old('role', $user->role) == 'partner' ? 'selected' : '' }}>الشيخ</option>
                    </select>
                    @error('role')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3" 
                              ...
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('notes', $user->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end space-x-3 space-x-reverse mt-8 pt-6 border-t border-gray-200">
                <a href="{{ route('users.index') }}" class="px-6 py-3 border border-red-300 text-blue-700 rounded-lg font-medium hover:bg-gray-50">
                    إلغاء
                </a>
        ...
        <div class="mt-8 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-blue-800 mb-4">تغيير كلمة المرور</h3>
            <form action="{{ route('users.change-password', $user) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @csrf
                <div>
                ...
                <div class="md:col-span-2">
                    <button type="submit" class="px-6 py-3 bg-yellow-500 hover:bg-yellow-600 text-blue rounded-lg font-medium">
                        تغيير كلمة المرور
                    </button>
                </div>
            </form>
