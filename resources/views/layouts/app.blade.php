...
<body class="font-sans antialiased bg-blue-50">
    <div class="min-h-screen flex">
            ...
            <nav class="flex-1 overflow-y-auto">
                <div class="px-6 space-y-4 py-4">
                    ...
                    <div class="mb-6">
                        ...
                        <a href="{{ route('areas.index') }}" class="flex items-center px-4 py-3.5 text-gray-700 rounded-xl hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 mt-2 {{ request()->routeIs('areas.*') ? 'bg-blue-50 text-blue-700 border-r-4 border-blue-500 shadow-sm' : '' }}">
                            <svg class="w-5 h-5 mr-3 {{ request()->routeIs('areas.*') ? 'text-blue-600' : 'text-gray-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10l1.664-1.664a4 4 0 015.657 0L12 10l1.679-1.679a4 4 0 015.657 0L21 10v7a2 2 0 01-2 2H5a2 2 0 01-2-2v-7z"></path>
                            </svg>
        ...
        <div class="flex-1 flex flex-col min-h-screen">
            ...
            <main class="flex-1 overflow-y-auto bg-blue-50 p-6">
                @if(session('success'))
                    <div id="toast-success" class="mb-4 max-w-xl ml-auto bg-green-50 border border-green-200 text-green-800 rounded px-4 py-3 shadow">
                        {{ session('success') }}
                    </div>
                @endif
                @if(session('error'))
                    <div id="toast-error" class="mb-4 max-w-xl ml-auto bg-red-50 border border-red-200 text-red-800 rounded px-4 py-3 shadow">
                        {{ session('error') }}
                    </div>
                @endif
                @yield('content')
    ...
    <script>
    setTimeout(function(){
      var s=document.getElementById('toast-success'); if(s){ s.style.transition='opacity 300ms'; s.style.opacity='0'; setTimeout(()=>s.remove(), 350); }
      var e=document.getElementById('toast-error'); if(e){ e.style.transition='opacity 300ms'; e.style.opacity='0'; setTimeout(()=>e.remove(), 350); }
    }, 2500);
    </script>
</body>
</html>
